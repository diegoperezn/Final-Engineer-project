...
'''<summary>
'''This is a test class for ArticuloRepositorioTest and is intended
'''to contain all ArticuloRepositorioTest Unit Tests
'''</summary>
<TestClass()> _
...
#End Region
    ...
    Dim target As ArticuloRepositorio = RepositorioFactory.articuloRepositorio
    ...
    Public Sub guardarArticuloTest()
        Dim precios As New List(Of ListaPrecios)
        precios.Add(New ListaPrecios(Nothing, DateTime.Now, Nothing, 1, Nothing, False))
        Dim articulo As New Articulo(Nothing, "com test", 1, 10, New TipoPrenda(3), New Diseño(2), precios)
    ...
    Public Sub actualizaArticuloTest()
        ...
        articuloBase.listaPrecios(0).fechaHasta = fecha
        articuloBase.listaPrecios.Add(New ListaPrecios(Nothing, fecha, Nothing, 2, articuloBase, False))
        ...
        Assert.AreEqual(10, articuloBaseEditado.produccion)
    ...
    '''<summary>
    '''A test for obtenerArticulosFrecuentes
    '''</summary>
    <TestMethod()> _
    Public Sub obtenerArticulosFrecuentesTest()
        Dim lista As List(Of Articulo) = target.obtenerArticulosFrecuentes(1)
        ...
        Assert.AreEqual(Long.Parse(1), lista(2).tipoPrenda.tipoPrenda)
        Assert.AreEqual("Campera Armada", lista(2).tipoPrenda.descripcion)
        Assert.AreEqual(Long.Parse(1), lista(2).diseño.idDiseño)
        Assert.AreEqual("diseño1", lista(2).diseño.nombre)
        Assert.AreEqual("com1", lista(2).comentario)
        Assert.AreEqual(Double.Parse(2.1000000000000001), lista(2).precioActual)
        Assert.AreEqual(2, lista(2).listaPrecios.Count)
        Assert.AreEqual(Double.Parse(1.1000000000000001), lista(2).listaPrecios(0).precio)
    End Sub
End Class
